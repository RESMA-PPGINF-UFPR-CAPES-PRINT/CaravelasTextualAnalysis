...
modelp = tf.keras.models.load_model(path_to_model, compile=False)
modelp.compile(optimizer=optimizer, loss=loss, metrics=metrics)
result = modelp.evaluate(val_ds, verbose=0, return_dict=True)
f1 = 2 * (result['precision']*result['recall'] / (result['precision']+result['recall']))
print(ID, 'MODEL PRECISION',RODADA,result, 'f1', f1)

path_to_model = '/home/hfrocha/envRLBERT/treino/modelos/bestloss'+ID+RODADA
modell = tf.keras.models.load_model(path_to_model, compile=False)
modell.compile(optimizer=optimizer, loss=loss, metrics=metrics)
result = modell.evaluate(val_ds, verbose=0, return_dict=True)
f1 = 2 * (result['precision']*result['recall'] / (result['precision']+result['recall']))
print(ID, 'MODEL LOSS',result, 'f1', f1)
"""
"""
path_to_model = '/home/hfrocha/envRLBERT/treino/modelos/modelo01'+ID+RODADA
model1 = tf.keras.models.load_model(path_to_model, compile=False)

path_to_model = '/home/hfrocha/envRLBERT/treino/modelos/modelo02'+ID+RODADA
model2 = tf.keras.models.load_model(path_to_model, compile=False)

path_to_model = '/home/hfrocha/envRLBERT/treino/modelos/modelo_v01'+ID+RODADA
modelv1 = tf.keras.models.load_model(path_to_model, compile=False)

path_to_model = '/home/hfrocha/envRLBERT/treino/modelos/modelo_v02'+ID+RODADA
modelv2 = tf.keras.models.load_model(path_to_model, compile=False)
"""
if METRIC=='f1':
